...
const User_Model = require("../../../../components/users/models/User_Model");
...
router.post("/insert-users", async (req, res, next) => {
  try {
     const users = User_Model.insertMany(usersFake);
     console.log("🚀 ~ file: Register.js ~ line 69 ~ router.post ~ users", users)
  } catch (error) {
    // next(error);
  }
});
